      ...
      <div className="container px-4 md:px-6">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {categories.map((category, index) => (
            <div
              key={category.title}
              className="group relative overflow-hidden rounded-2xl h-[400px] animate-on-scroll"
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-6 text-white">
                <h3 className="text-2xl font-bold transform transition-transform group-hover:-translate-y-2">
                  {category.title}
                </h3>
                <p className="mb-4 text-sm/relaxed opacity-0 transform translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                  {category.description}
                </p>
                <Button 
                  variant="outline" 
                  className="w-full text-white border-white hover:bg-white hover:text-gray-900 transition-colors"
